  ...
  useEffect(() => {
    const loadComplaints = async () => {
      try {
        ...
        const complaintsArray = Array.isArray(apiComplaints)
          ? apiComplaints
          : [];
          ...
          <div className="space-y-4">
            {filteredComplaints.map((complaint) => (
              <div
                ...
                className="bg-brand-gray rounded-xl p-6 border border-white/10 hover:border-brand-red/50 transition-all"
              >
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border ${getPriorityColor(complaint.priority)}`}
                            >
                              {complaint.priority.toUpperCase()}
                            </span>
                            <span
                              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold ${getStatusColor(complaint.status)}`}
                            >
