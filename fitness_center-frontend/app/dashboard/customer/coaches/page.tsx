...
const FindCoachesPage = () => {
  ...
  useEffect(() => {
    loadCoaches();
    const interval = setInterval(loadCoaches, 30000); // Refresh every 30 seconds
  ...
  const loadCoaches = async () => {
    try {
      ...
      // Get only approved coaches (backend filters automatically for non-admin users)
      const coachesData = await CoachesAPI.getAll();
      console.log('Coaches loaded:', coachesData);
      
      // Filter out coaches with invalid MongoDB ObjectIds
      const validCoaches = coachesData.filter((coach: any) => {
        const id = coach._id || coach.id;
        const isValid = id && id.length === 24 && /^[a-f\d]{24}$/i.test(id);
        if (!isValid) {
          console.warn('Filtered out coach with invalid ID:', coach.name, id);
        }
        return isValid;
      });
  ...
  const specializations = ['All', 'Strength Training', 'HIIT', 'Yoga', 'Weight Loss', 'Sports Performance', 'Pilates', 'Boxing', 'Cardio', 'Nutrition Coaching'];

  const filteredCoaches = coaches.filter(coach => {
    const matchesSearch = coach.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         coach.specializations?.some(s => s.toLowerCase().includes(searchQuery.toLowerCase()));
    const matchesSpec = selectedSpecialization === 'All' || coach.specializations?.includes(selectedSpecialization);
    // For availability, we'll skip it for now since we don't have that field in the database
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Browse {coaches.length} expert trainers</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="bg-brand-gray border border-white/10 rounded-xl p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {['All', 'Available Today', 'Available Tomorrow'].map((avail) => (
                    <button
                      key={avail}
        ...
        <div className="mb-6 flex items-center justify-between">
          <p className="text-gray-400">
            Showing <span className="text-white font-semibold">{filteredCoaches.length}</span> coaches
          </p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCoaches.map((coach) => (
            ...
            >
              {/* Coach Header */}
              <div className="relative bg-gradient-to-br from-brand-red to-red-900 p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1 bg-black/30 px-3 py-1 rounded-full backdrop-blur-sm">
                    ...
                    <span className="text-sm text-gray-300">({coach.reviewCount || 0})</span>
                  </div>
                </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(coach.specializations && coach.specializations.length > 0) ? (
                      ...
                      <span className="text-xs text-gray-500">No specializations listed</span>
                    )}
                  </div>
                </div>
                ...
                <p className="text-sm text-gray-400 line-clamp-2">
                  {coach.bio || 'Professional fitness coach dedicated to helping you achieve your goals.'}
                </p>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="font-bold">LKR {coach.hourlyRate || 0}/hr</p>
                    </div>
                  </div>
                </div>
                ...
                {coach.certifications && coach.certifications.length > 0 && (
                  <div className="pt-2">
                    ...
                    <p className="text-xs text-gray-500">{coach.certifications.join(', ')}</p>
                  </div>
                )}
