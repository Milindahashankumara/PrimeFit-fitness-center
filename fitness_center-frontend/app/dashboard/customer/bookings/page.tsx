...
interface Booking {
  ...
  duration?: number;
  status: 'upcoming' | 'completed' | 'cancelled' | 'pending' | 'accepted' | 'rejected' | 'rescheduled';
  location: string;
  price: number;
  sessionType: string;
  message?: string;
...
const BookingsPage = () => {
  ...
  useEffect(() => {
    const loadBookings = async () => {
      ...
      try {
        ...
        const transformedBookings: Booking[] = bookingsArray.map((booking: any, index) => {
          ...
          return {
            ...
            id: typeof bookingId === 'string' ? parseInt(bookingId.slice(-8), 16) : bookingId,
            _id: booking._id, // Ensure MongoDB _id is preserved
            coachName: booking.coachName || 'Unknown Coach',
            coachImage: (booking.coachName || 'UK').split(' ').map((n: string) => n[0]).join(''),
            status: booking.status === 'rescheduled' ? 'rescheduled' :
                    booking.status === 'accepted' ? 'upcoming' : 
  ...
  const filteredBookings = allBookings.filter(booking => {
    ...
    if (activeTab === 'upcoming') {
      return booking.status === 'upcoming' || booking.status === 'rescheduled' || booking.status === 'accepted';
    }
  ...
  const confirmCancellation = async () => {
    ...
    try {
      ...
      const updatedBooking = await BookingsAPI.update(selectedBooking._id, { status: 'cancelled' });
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Manage your training sessions</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {authError && (
          <div className="mb-6 bg-red-500/20 border border-red-500/50 rounded-xl p-6 text-center">
            ...
            <p className="text-gray-300 mb-4">You need to log in to view your bookings. Redirecting to login page...</p>
              ...
              >
                <div className="p-6">
                    ...
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h3 className="text-xl font-bold">{booking.coachName}</h3>
                        ...
                        {/* Reschedule Notice */}
                        {booking.status === 'rescheduled' && booking.rescheduledBy === 'coach' && (
                          <div className="bg-yellow-500/10 border border-yellow-500/30 p-3 rounded-lg mb-3">
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1">
                                <p className="text-sm font-semibold text-yellow-400 mb-1">Coach Rescheduled This Session</p>
                                <p className="text-xs text-gray-300 mb-2">
                                  <span className="line-through text-gray-500">Original: {booking.originalDate} at {booking.originalTime}</span><br />
                                  <span className="text-green-400 font-semibold">New: {booking.date} at {booking.time}</span>
                                </p>
                                {booking.rescheduleReason && (
                                  <p className="text-xs text-gray-400 italic">"{booking.rescheduleReason}"</p>
                                )}
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="grid md:grid-cols-4 gap-4 mt-4">
                          ...
                          <div className="flex items-center gap-2">
                            {booking.location === 'Online' ? (
                              <Video className="text-brand-red shrink-0" size={18} />
                            ) : (
                          ...
                          <div className="flex items-center gap-2">
                            <User className="text-brand-red shrink-0" size={18} />
                            <div>
                              <p className="text-xs text-gray-400">Type</p>
                              <p className="font-semibold text-sm capitalize">{booking.type}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="bg-black/40 p-3 rounded-lg mb-4 inline-block">
                    ...
                    <span className="text-brand-red font-bold text-lg">LKR {booking.price}</span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    {booking.status === 'completed' && (
                      <>
                        ...
                        <button 
                          onClick={() => router.push(`/dashboard/customer/coaches/${booking.id}`)}
      ...
      {showCancelModal && selectedBooking && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-brand-gray rounded-2xl max-w-md w-full p-6 border border-white/10">
            ...
            <h2 className="text-2xl font-bold text-center mb-2">Cancel Booking?</h2>
            <p className="text-gray-400 text-center mb-6">
              Are you sure you want to cancel your session with {selectedBooking.coachName} on {formatDate(selectedBooking.date)}?
            </p>
