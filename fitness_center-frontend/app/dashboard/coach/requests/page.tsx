...
const BookingRequestsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'pending' | 'accepted' | 'rejected' | 'completed' | 'all'>('pending');
  ...
  const handleConfirmReschedule = async () => {
    ...
    try {
      ...
      setSuccessMessage('Session rescheduled successfully! Customer will be notified.');
      setShowSuccess(true);
  ...
  const handleCompleteSession = async (request: BookingRequest) => {
    if (confirm(`Mark session with ${request.clientName} as completed?`)) {
      try {
        ...
        setSuccessMessage('Session marked as completed! Customer can now provide feedback.');
        setShowSuccess(true);
        setTimeout(() => setShowSuccess(false), 3000);
      } catch (error) {
  ...
  const handleConfirmAction = async () => {
    if (selectedRequest) {
      try {
        ...
        setSelectedRequest(null);
        setResponseMessage('');
        setSuccessMessage(`Booking request ${actionType === 'accept' ? 'accepted' : 'rejected'} successfully!`);
        setShowSuccess(true);
        setTimeout(() => setShowSuccess(false), 3000);
      } catch (error) {
  ...
  const getSessionTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'group': return <User size={18} />;
      case 'online': return <Video size={18} />;
  ...
  const getPendingCount = () => bookingRequests.filter(r => r.status === 'pending').length;
  const getAcceptedCount = () => bookingRequests.filter(r => r.status === 'accepted').length;
  const getRejectedCount = () => bookingRequests.filter(r => r.status === 'rejected').length;
  const getCompletedCount = () => bookingRequests.filter(r => r.status === 'completed').length;
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Review and respond to session requests</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-brand-gray rounded-xl p-6 border border-white/10">
            ...
            <p className="text-3xl font-bold">
              LKR {bookingRequests.filter(r => r.status === 'accepted' || r.status === 'completed').reduce((sum, r) => sum + r.price, 0)}
            </p>
        ...
        <div className="space-y-4">
          {filteredRequests.length === 0 ? (
            <div className="bg-brand-gray rounded-2xl p-12 border border-white/10 text-center">
              ...
              <h3 className="text-xl font-bold mb-2">No {activeTab !== 'all' ? activeTab : ''} requests</h3>
              <p className="text-gray-400">
                {activeTab === 'pending' && 'You have no pending booking requests at the moment.'}
                {activeTab === 'accepted' && 'You haven\'t accepted any bookings yet.'}
                {activeTab === 'rejected' && 'You haven\'t rejected any bookings.'}
            ...
            filteredRequests.map((request) => (
              <div key={request._id} className="bg-brand-gray rounded-2xl p-6 border border-white/10">
                  ...
                  <div className="flex-1">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <div className="w-12 h-12 bg-brand-red rounded-full flex items-center justify-center font-bold">
                            {request.clientName?.split(' ').map(n => n[0]).join('') || 'UK'}
                          </div>
                          <div>
                            <h3 className="text-xl font-bold">{request.clientName || 'Unknown Customer'}</h3>
                            <p className="text-sm text-gray-400">{request.clientEmail || 'No email'}</p>
                          </div>
                        </div>
                      </div>
                      <div className={`px-3 py-1 rounded-lg ${getStatusColor(request.status)}`}>
                    ...
                    <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                      ...
                      <div className="bg-black/40 p-3 rounded-lg">
                        ...
                        <p className="font-semibold">{request.time} ({request.duration} min)</p>
                      </div>
                      ...
                      <div className="bg-black/40 p-3 rounded-lg">
                        ...
                        <p className={`font-semibold capitalize px-2 py-1 rounded ${getSessionTypeColor(request.sessionType)}`}>
                          {request.sessionType}
                        </p>
                      </div>
                      ...
                      <div className="bg-black/40 p-3 rounded-lg">
                        ...
                        <p className="font-semibold text-brand-red">LKR {request.price}</p>
                      </div>
                    </div>
                    ...
                    {request.message && (
                      <div className="bg-blue-500/10 border border-blue-500/30 p-3 rounded-lg mb-3">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-blue-400 font-semibold">Client Message</span>
                        </div>
                  ...
                  {request.status === 'rescheduled' && (
                    <div className="flex flex-col gap-2 lg:min-w-[200px]">
                      <div className="bg-blue-500/20 border border-blue-500/50 px-4 py-3 rounded-lg">
                        <p className="text-xs text-blue-400 mb-1">Rescheduled From:</p>
                        <p className="text-sm font-semibold">{request.originalDate} at {request.originalTime}</p>
                        {request.rescheduleReason && (
                          <p className="text-xs text-gray-400 mt-2">{request.rescheduleReason}</p>
                        )}
                      </div>
                    </div>
                  )}
          ...
          <div className="bg-brand-gray rounded-2xl max-w-lg w-full p-6 border border-white/10">
            ...
            <div className="bg-black/40 p-4 rounded-lg mb-4">
              ...
              <p className="text-sm text-gray-300 mt-1">
                {selectedRequest.duration} min • {selectedRequest.sessionType} session • LKR {selectedRequest.price}
              </p>
            </div>
            ...
            {actionType === 'accept' ? (
              <div className="bg-green-500/10 border border-green-500/30 p-4 rounded-lg mb-4">
                <p className="text-green-400 text-sm">
                  By accepting this request, you confirm that you're available for this session. 
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-brand-gray rounded-2xl max-w-lg w-full p-6 border border-white/10">
            ...
            <div className="space-y-4 mb-4">
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-2">Reason for Rescheduling</label>
                <textarea
            ...
            <div className="bg-blue-500/10 border border-blue-500/30 p-4 rounded-lg mb-4">
              <p className="text-blue-400 text-sm">
                The customer will be notified about the new schedule and can accept or reject the change.
              </p>
            </div>
